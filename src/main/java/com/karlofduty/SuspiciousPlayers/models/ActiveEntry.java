...
public class ActiveEntry extends PlayerEntry {
    ...
    public Component edit(final Connection connection, String message) {
        try (PreparedStatement statement = connection.prepareStatement(ActiveEntry.EDIT)) {
            ...
            statement.setTimestamp(2, this.createdTime); // Prevent mysql from automatically updating the created_time to the
            // current time
    ...
    public Component getInteractiveMessage() {
        ...
        return Component.empty().append(Component.text("[", NamedTextColor.GREEN)).append(Component.text("-", NamedTextColor.GOLD).clickEvent(net.kyori.adventure.text.event.ClickEvent.runCommand("/susparchive " + id)).hoverEvent(net.kyori.adventure.text.event.HoverEvent.showText(Component.text("Archive entry.", NamedTextColor.GOLD)))).append(Component.text("] [", NamedTextColor.GREEN)).append(Component.text("✎").clickEvent(net.kyori.adventure.text.event.ClickEvent.suggestCommand("/suspedit " + id + " ")).hoverEvent(HoverEvent.showText(Component.text("Edit entry.", NamedTextColor.GOLD)))).append(Component.text("] [", NamedTextColor.GREEN)).append(Component.text(displayDateFormat.format(createdTime), NamedTextColor.YELLOW)).append(Component.text("] Reported by: ", NamedTextColor.GREEN)).append(reporterComponent).append(Component.text("\n" + entry + "\n", NamedTextColor.GREEN));
    }
}
