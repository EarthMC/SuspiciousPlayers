...
public class ArchivedEntry extends PlayerEntry {
  ...
  public static final String INSERT =
      "INSERT INTO archived_entries(archived_time, archiver_uuid, created_time, creator_uuid, suspicious_uuid, entry) VALUES (?,?,?,?,?,?)";
  public static final String SELECT_PLAYER =
      "SELECT * FROM archived_entries WHERE suspicious_uuid = ? ORDER BY created_time LIMIT ?;";
    ...
    try (PreparedStatement insertStatement = c.prepareStatement(DeletedEntry.INSERT)) {
      ...
      try (PreparedStatement statement = c.prepareStatement(DELETE)) {
        ...
        return Component.text(
            "Error occurred while deleting archived entry from database: " + e.getMessage(),
            NamedTextColor.RED);
      }
    } catch (SQLException e) {
      SuspiciousPlayers.plugin().logger().warn("SQLException while deleting entry:", e);
      return Component.text(
          "Error occurred while inserting entry into deleted table: " + e.getMessage(),
          NamedTextColor.RED);
    }
  ...
  public Component unarchive(Connection c) {
    try (PreparedStatement insertStatement = c.prepareStatement(ActiveEntry.INSERT)) {
      ...
      return Component.text(
          "Error occurred while inserting entry into active table: " + e.getMessage(),
          NamedTextColor.RED);
    }
    ...
    return Component.empty()
        ...
        .append(
            Component.text("+", NamedTextColor.GREEN)
                .clickEvent(
                    net.kyori.adventure.text.event.ClickEvent.runCommand("/suspunarchive " + id))
                .hoverEvent(
                    net.kyori.adventure.text.event.HoverEvent.showText(
                        Component.text("Unarchive this entry.", NamedTextColor.GREEN))))
        .append(Component.text("] [", NamedTextColor.DARK_GRAY))
        ...
        .append(
            Component.text("x", NamedTextColor.RED)
                ...
                .hoverEvent(
                    HoverEvent.showText(Component.text("Delete this entry.", NamedTextColor.RED))))
        .append(Component.text("] [", NamedTextColor.DARK_GRAY))
