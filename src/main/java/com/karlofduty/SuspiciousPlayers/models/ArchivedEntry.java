...
public class ArchivedEntry extends PlayerEntry {
    ...
    /**
     * Selects a single row from the archived entries table in the database and returns an
     * ArchivedEntry object representing it
     *
    ...
    public static ArchivedEntry select(Connection c, int id) {
        try (PreparedStatement selectStatement = c.prepareStatement(ArchivedEntry.SELECT)) {
            ...
            if (resultSet.next()) return new ArchivedEntry(resultSet);
    ...
    public Component delete(Connection c, String deleterUUID) {
        try (PreparedStatement insertStatement = c.prepareStatement(DeletedEntry.INSERT)) {
            ...
            try (PreparedStatement statement = c.prepareStatement(DELETE)) {
                ...
                SuspiciousPlayers.plugin().logger().warn("SQLException while deleting entry:", e);
    ...
    public Component getInteractiveMessage() {
        ...
        return Component.empty().append(Component.text("[", NamedTextColor.DARK_GRAY)).append(Component.text("+", NamedTextColor.GREEN).clickEvent(net.kyori.adventure.text.event.ClickEvent.runCommand("/suspunarchive " + id)).hoverEvent(net.kyori.adventure.text.event.HoverEvent.showText(Component.text("Unarchive this entry.", NamedTextColor.GREEN)))).append(Component.text("] [", NamedTextColor.DARK_GRAY)).append(Component.text(displayDateFormat.format(createdTime), NamedTextColor.GRAY)).append(Component.text("] Reported by: ", NamedTextColor.DARK_GRAY)).append(reporterComponent).append(Component.text("\n[", NamedTextColor.DARK_GRAY)).append(Component.text("x", NamedTextColor.RED).clickEvent(ClickEvent.runCommand("/suspdelete " + id)).hoverEvent(HoverEvent.showText(Component.text("Delete this entry.", NamedTextColor.RED)))).append(Component.text("] [", NamedTextColor.DARK_GRAY)).append(Component.text(displayDateFormat.format(archivedTime), NamedTextColor.GRAY)).append(Component.text("] Archived by: ", NamedTextColor.DARK_GRAY)).append(archiverComponent).append(Component.text("\n" + entry + "\n", NamedTextColor.DARK_GRAY));
    }
}
