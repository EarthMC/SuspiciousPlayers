...
public abstract class PlayerEntry {
    ...
    /**
     * Gets an interactive message for a list entry with different buttons and actions depending on
     * the type of entry.
     *
    ...
    /**
     * Gets a TextComponent containing the name and a popup with name history of a player. MAKE SURE
     * THIS IS ALWAYS CALLED ASYNCHRONOUSLY!
     *
    ...
    public static Component getNameComponent(UUID uuid, NamedTextColor color) {
        ...
        try (Connection connection = SuspiciousPlayers.plugin().getConnection(); PreparedStatement statement = connection.prepareStatement("select * from player_history where uuid = ?")) {
            ...
            for (Map.Entry<String, String> entry : usernameHistory.entrySet()) {
                popup = popup.append(Component.text("\n" + String.format("    %-32s", entry.getValue()), NamedTextColor.GRAY)).append(Component.text(String.format("%-20s", entry.getKey()), NamedTextColor.DARK_GRAY).decorate(TextDecoration.ITALIC));
            }

            popup = popup.append(Component.text("\n\nClick to open susplist for this player.", NamedTextColor.YELLOW));

            // Builds the final TextComponent and returns it
            return Component.text(usernameHistory.firstEntry().getValue(), color).hoverEvent(net.kyori.adventure.text.event.HoverEvent.showText(Component.text("Known aliases:", NamedTextColor.DARK_GRAY).append(popup))).clickEvent(ClickEvent.runCommand("/susplist " + usernameHistory.firstEntry().getValue()));
        } catch (Exception e) {
            ...
            return Component.text(uuid.toString(), color).hoverEvent(HoverEvent.showText(Component.text("Known aliases:\n", NamedTextColor.DARK_GRAY).append(Component.text("ERROR: " + e.getMessage(), NamedTextColor.RED))));
        }
    }
}
