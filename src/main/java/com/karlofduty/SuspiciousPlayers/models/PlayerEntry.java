  ...
  public static Component getNameComponent(UUID uuid, NamedTextColor color) {
    ...
    try (Connection connection = SuspiciousPlayers.plugin().getConnection();
        PreparedStatement statement =
            connection.prepareStatement("select * from player_history where uuid = ?")) {
      ...
      for (Map.Entry<String, String> entry : usernameHistory.entrySet()) {
        popup =
            popup
                ...
                .append(
                    Component.text(String.format("%-20s", entry.getKey()), NamedTextColor.DARK_GRAY)
      ...
      return Component.text(usernameHistory.firstEntry().getValue(), color)
          .hoverEvent(
              net.kyori.adventure.text.event.HoverEvent.showText(
                  Component.text("Known aliases:", NamedTextColor.DARK_GRAY).append(popup)))
          .clickEvent(
              ClickEvent.runCommand("/susplist " + usernameHistory.firstEntry().getValue()));
    } catch (Exception e) {
