    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String @NotNull [] args) {
        ...
        plugin.server.getAsyncScheduler().runNow(plugin, t -> {
            try (Connection c = plugin.getConnection()) {
                ...
                if (entries.isEmpty()) {
                    sender.sendMessage(Component.text("No online users have any active entries.", NamedTextColor.RED));
                    return;
                }
                ...
                for (Map.Entry<String, LinkedList<ActiveEntry>> playerEntries : entries.entrySet()) {
                    try {
                        message.append(Component.text("----- Displaying entries for ", NamedTextColor.GOLD));
                        message.append(PlayerEntry.getNameComponent(UUID.fromString(playerEntries.getKey()), NamedTextColor.YELLOW));
                ...
                sender.sendMessage(Component.text("Error occurred while interacting with the database. " + e.getMessage(), NamedTextColor.RED));
                plugin.logger().warn("Exception in susponline:", e);
            }
        });
        return true;
