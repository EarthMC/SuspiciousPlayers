...
public class TPHandler {
    ...
    public static Player nextSusp(Player player) {
        ...
        try (Connection c = SuspiciousPlayers.plugin().getConnection()) {
            ...
            SuspiciousPlayers.plugin().logger().warn("SQLException while loading entries for nextSusp:", e);
            return null;
        }
    }
    ...
    public static Player prevSusp(Player player) {
        ...
        try (Connection c = SuspiciousPlayers.plugin().getConnection()) {
            ...
            SuspiciousPlayers.plugin().logger().warn("SQLException while loading entries for prevSusp:", e);
            return null;
        }
    }
    ...
    private static Component getLeftArrow(boolean suspOnly) {
        return Component.text().append(Component.text("[", NamedTextColor.DARK_GRAY)).append(Component.text("<", NamedTextColor.GOLD).clickEvent(ClickEvent.runCommand(suspOnly ? "/suspprev" : "/tpprev")).hoverEvent(HoverEvent.showText(Component.text("Previous", NamedTextColor.YELLOW)))).append(Component.text("] ", NamedTextColor.DARK_GRAY)).build();
    }
    ...
    private static Component getRightArrow(boolean suspOnly) {
        return Component.text().append(Component.text(" [", NamedTextColor.DARK_GRAY)).append(Component.text(">", NamedTextColor.GOLD).clickEvent(ClickEvent.runCommand(suspOnly ? "/suspnext" : "/tpnext")).hoverEvent(HoverEvent.showText(Component.text("Next", NamedTextColor.YELLOW)))).append(Component.text("]", NamedTextColor.DARK_GRAY)).build();
    }
    ...
    public static Component getTPStatus(Player player, boolean suspOnly) {
        return getLeftArrow(suspOnly).append(PlayerEntry.getNameComponent(player.getUniqueId(), NamedTextColor.YELLOW)).append(getRightArrow(suspOnly));
    }
}
